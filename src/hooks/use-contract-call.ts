...
export function useContractCall() {
  ...
  const call = useCallback(async (params: ContractCallParams): Promise<ContractCallResult> => {
    setIsLoading(true);
    setError(null);
    setIsSuccess(false);
    ...
    try {
      ...
      const callError = err instanceof Error ? err : new Error('Contract call failed');
      setError(callError);
      throw callError;
    } finally {
      setIsLoading(false);
